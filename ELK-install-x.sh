...
# If you plan to use also winlogbeat plugin then change hostname to 0.0.0.0
...
cp /etc/auditbeat/auditbeat.yml /etc/auditbeat/backup-auditbeat.yml
cat <<EOC | sudo su
cat <<EOT > /etc/auditbeat/auditbeat.yml
auditbeat.modules:
- module: auditd
  audit_rules: |
...
sudo auditbeat export template > /etc/auditbeat/auditbeat.template.json
...
sudo /usr/share/elasticsearch/bin/elasticsearch-plugin install ingest-geoip --batch
...
echo "Auditbeat:" /etc/auditbeat/auditbeat.yml
